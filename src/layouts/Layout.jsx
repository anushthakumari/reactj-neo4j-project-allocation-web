...
const Layout = ({ children }) => {
	...
	const { user, saveUser } = useAuthState();

	const routes = getRoutes(user?.role_id);
		...
		<div>
			<nav className="relative px-4 py-4 flex justify-between items-center bg-white">
				<Link
					...
					{user ? (
						<h1 className="capitalize text-lg">
							hello, {user.full_name}, the {user.role_name}
						</h1>
					) : (
						""
					)}
				...
				<ul className="hidden absolute top-1/2 left-1/2 transform -translate-y-1/2 -translate-x-1/2 lg:flex lg:mx-auto lg:flex lg:items-center lg:w-auto lg:space-x-6">
					{routes.map((v) => (
						<React.Fragment key={v.to}>
							<li>
								<Link
									className={`text-sm ${
										location.pathname.toLowerCase() === v.to
											? "text-blue-600"
											: ""
...
function getRoutes(role_id) {
	switch (role_id) {
		case roles.USER:
			return userRoutes;

		case roles.SUPERVISIOR:
			return supRoutes;
